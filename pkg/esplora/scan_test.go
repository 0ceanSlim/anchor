...
func buildAnchorScript(asset0, asset1 [32]byte, feeNum, feeDen uint16) string {
	// OP_RETURN (6a) + PUSH73 (49) + payload
	var payload [73]byte
	copy(payload[0:5], "ANCHR")
	// Assets are stored in internal byte order (reversed from display).
...
func TestParseAnchorOutput(t *testing.T) {
	// Use known asset bytes (internal order).
	...
	// Display hex should be the reverse of internal bytes.
